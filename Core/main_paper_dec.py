...
######################################################
   ...
   'Price_DT_mod']
    ...
    data_input.T_var=data_input.T_var.ffill()
    ...
    conf_aux=[True,True,False,False]#[Batt,HP,TS,DHW]
...
def main():
    ...
    base_dir = Path(__file__).resolve().parent.parent  # Adjust if your script is in a subdirectory
    filename = base_dir / 'Output' / 'aggregated_results.csv'
    filename.parent.mkdir(parents=True, exist_ok=True)

    if not filename.exists():
        cols = ['E_PV_batt','E_PV_bu','E_PV_budhw','E_PV_curt','E_PV_grid','E_PV_hp','E_PV_hpdhw','E_PV_load',
                'E_batt_bu','E_batt_budhw','E_batt_hp','E_batt_hpdhw','E_batt_load','E_bu','E_budhw','E_char',
                'E_cons','E_dis','E_grid_batt','E_grid_bu','E_grid_budhw','E_grid_hp','E_grid_hpdhw','E_grid_load',
                'E_hp','E_hpdhw','E_loss_Batt','E_loss_conv','E_loss_inv','E_loss_inv_PV','E_loss_inv_batt',
                'E_loss_inv_grid','Q_dhwst_hd','Q_hp_sh','Q_hp_ts','Q_loss_dhwst','Q_loss_ts','Q_ts','Q_ts_delta',
                'Q_ts_sh','T_dhwst','T_ts','E_demand','E_PV','Req_kWh','Req_kWh_DHW','Set_T','Temp','Temp_supply',
                'Temp_supply_tank','T_aux_supply','COP_tank','COP_SH','COP_DHW','E_demand_hp_pv_dhw','E_demand_hp_pv',
                'E_demand_pv','TSC','DSC','ISC','CU','EFC_nolifetime','BS','LS1','LS2','LS3','LS4','LS5','PS1','PS2',
                'PS3','PS4','PS5','App_comb','conf','Capacity','Tech','PV_nom','cluster','hh','country','quartile',
                'cases','house_type','HP','SOC_mean','P_drained_max','P_injected_max','last_cap','cap_fading',
                'last_SOH','cycle_to_total','Bill','Bill_noexp','P_max_mean']
        ...
        aux = df_done.groupby([df_done.Capacity, df_done.App_comb, df_done.Tech, df_done.PV_nom,
                            df_done.cluster, df_done.hh, df_done.country, df_done.cases,
                            df_done.conf, df_done.HP, df_done.house_type]).size().reset_index()
    ...
    try:
        ...
        dct={'Capacity':[7],'App_comb':[0],'Tech':['NMC',],'PV_nom':[4.8],'country':['CH'],'cases':['mean'],'conf':[1],'house_type':['SFH15'],'HP':['AS'],'hh':[110143956137]}
