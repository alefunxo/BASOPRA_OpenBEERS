...
def Concrete_model(Data):
    ...
    #EV
    m.SOC_init_EV=en.Param(initialize=Data['E_EV_start'])   # in kWh
    m.SOC_min_EV=en.Param(initialize=Data['Batt_EV'].SOC_min) # in kWh
    m.SOC_max_EV=en.Param(initialize=Data['Batt_EV'].SOC_max) # in kWh no aging for now

    m.Capacity_EV=en.Param(initialize=Data['Batt_EV'].Capacity) # in kWh
    m.Efficiency_EV=en.Param(initialize=Data['Batt_EV'].Efficiency)
    m.Batt_dis_max_EV=en.Param(initialize=-Data['Batt_EV'].P_max_dis)
    m.Batt_char_max_EV=en.Param(initialize=Data['Batt_EV'].P_max_char)

    m.EV_P_max_home=en.Param(initialize=Data['EV_P_max_home'])#Maximum Energy charged in a timestep kW when parked at home
    m.EV_P_max_away=en.Param(initialize=Data['EV_P_max_away'])#Maximum Energy charged in a timestep kW when parked away
    m.EV_V2G=en.Param(initialize=Data['EV_V2G'])#Whether V2G is used (0 = No V2G, 1 = V2G)

    m.EV_home=en.Param(m.Time,initialize=Data['EV_home']) #Whether the EV is at home (1) or not (0)
    m.EV_away=en.Param(m.Time,initialize=Data['EV_away']) #Whether the EV is at parked away from home (1) or not (0)
    m.E_EV_trip=en.Param(m.Time,initialize=Data['E_EV_trip']) #Energy used for trips
    m.public_charging_price=en.Param(initialize=Data['public_charging_price'])
    m.minV2XSOC = en.Param(m.Time,initialize = Data['Batt_EV'].SOC_min) #Minimum SOC of the battery if V2G
    ...
    m.SOC_EV=en.Var(m.tm,bounds=(m.SOC_min_EV,m.SOC_max_EV),initialize=m.SOC_min_EV)
    ...
    # EV
    m.Bool_char_EV_rule_1_r=en.Constraint(m.Time,rule=Bool_char_EV_rule_1)
    m.Bool_char_EV_rule_2_r=en.Constraint(m.Time,rule=Bool_char_EV_rule_2)
    m.Bool_char_EV_rule_3_r=en.Constraint(m.Time,rule=Bool_char_EV_rule_3)
    m.Bool_char_EV_rule_4_r=en.Constraint(m.Time,rule=Bool_char_EV_rule_4)
    m.Batt_char_dis_EV_r=en.Constraint(m.Time,rule=Batt_char_dis_EV)
    m.Balance_Batt_EV_rule_r=en.Constraint(m.Time,rule=Balance_Batt_EV_rule)
    m.availability_rule_1_r=en.Constraint(m.Time,rule=availability_rule_1)
    m.availability_rule_2_r=en.Constraint(m.Time,rule=availability_rule_2)
    m.availability_rule_3_r=en.Constraint(m.Time,rule=availability_rule_3)
    m.E_char_EV_rule_r=en.Constraint(m.Time,rule=E_char_EV_rule)
    m.E_dis_EV_rule_r=en.Constraint(m.Time,rule=E_dis_EV_rule)
...
def Bool_char_EV_rule_1(m,i):
    ...
    '''
    bigM=500000
    return m.E_dis_EV[i]>=-bigM*m.Bool_dis_EV[i]

def Bool_char_EV_rule_2(m,i):
    '''
    Description
    -------
    ...
    '''
    bigM=500000
    return m.E_dis_EV[i]<=0+bigM*(1-m.Bool_char_EV[i])

def Bool_char_EV_rule_3(m,i):
    '''
    Description
    -------
    ...
    '''
    bigM=500000
    return m.E_char_EV[i]>=-bigM*m.Bool_char_EV[i]

def Bool_char_EV_rule_4(m,i):
    '''
    Description
    -------
    ...
    '''
    bigM=500000
    return m.E_char_EV[i]<=0+bigM*(1-m.Bool_dis_EV[i])
...
def Batt_char_dis_EV(m,i):
    ...
    '''
    return  m.Bool_char_EV[i]+m.Bool_dis_EV[i]==1

def Balance_Batt_EV_rule(m,i):
     '''
     Description
     -------
...
def availability_rule_1(m,i):
    ...
    """
    return m.E_char_EV[i] <= m.EV_home[i]*m.dt*min(m.EV_P_max_home,m.Batt_char_max_EV)
    
def availability_rule_2(m,i):
    """
    Ensures that car battery does not discharge at home when unavailable. (Still allows discharge to the car's powertrain!)
    """
    return m.E_dis_EV[i] <= m.EV_home[i]*m.dt*min(m.EV_P_max_home,m.Batt_dis_max_EV)
def availability_rule_3(m,i):
    """
    Ensures that car battery does not charge at home when unavailable.
    """
    return m.E_char_away[i] <= m.EV_away[i]*m.dt*min(m.EV_P_max_away,m.Batt_char_max_EV)

def E_char_EV_rule(m,i):
    '''
    Description
    -------
    Balance of energy charged into the battery from PV and grid.
    '''
    return(m.E_char_EV[i],((m.E_PV_batt_EV[i]+m.E_grid_batt_EV[i])))

def E_dis_EV_rule(m,i):
    '''
    Description
    -------
    Sets the maximum energy available to be discharged as the SOC - the minimum SOC.
    '''
    return(m.E_dis_EV[i]<=m.SOC_EV[i-1]-m.SOC_min_EV)
def E_dis_EV_rule2(m,i):
    '''
    Description
    -------
...
def min_V2X_SOC(m,i):
    ...
    if m.EV_V2G == 1: # V2G 
        ...
        return(m.E_batt_EV_grid[i]==0)
def def_storage_state_EV_rule(m, t):
    '''
    Description
    -------
    ...
    if t==-1:
        return(m.SOC_EV[t],m.SOC_init_EV)
    else:
...
def Bool_cons_rule_3(m,i):
    ...
    '''
    bigM=500000
    return (m.E_PV_grid[i]+m.E_batt_EV_grid[i])>=-bigM*(m.Bool_inj[i])
...
def Bool_cons_rule_4(m,i):
    ...
    '''
    bigM=500000
    return (m.E_PV_grid[i]+m.E_batt_EV_grid[i])<=0+bigM*(1-m.Bool_cons[i])
...
def Grid_cons_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_cons[i]==m.E_grid_batt[i]+m.E_grid_load[i]+m.E_loss_inv_grid[i]+m.E_grid_batt_EV[i]
    else:
        if not en.value(m.DHW):
            return m.E_cons[i]==m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_grid_bu[i]+m.E_loss_inv_grid[i]+m.E_grid_batt_EV[i]
        else:
            return m.E_cons[i]==m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_grid_hpdhw[i]+m.E_grid_bu[i]+m.E_grid_budhw[i]+m.E_loss_inv_grid[i]+m.E_grid_batt_EV[i]
...
def Balance_PV_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_PV[i]==m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]+m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_batt_EV[i]
    else:
        if not en.value(m.DHW):
            return m.E_PV[i] == m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_batt_EV[i]
        else:
            return m.E_PV[i] == m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_batt_EV[i]
...
def Balance_load_rule(m,i):
    ...
    '''
    return m.E_demand_el[i] == m.E_PV_load[i]+m.E_batt_load[i]+m.E_grid_load[i] + m.E_batt_EV_load[i]#-m.Inverter_power*0.5/100)
...
def Conv_losses_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_loss_conv[i] == ((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_batt_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff)
    else:
        if not en.value(m.DHW):
            return m.E_loss_conv[i] == ((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_PV_batt_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff)
        else:
            return m.E_loss_conv[i] == ((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_PV_batt_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff)
...
def Inv_losses_PV_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_loss_inv_PV[i] == (m.E_PV_grid[i]+m.E_PV_load[i]+m.E_PV_batt_EV[i])*(1-m.Inverter_eff)/(m.Inverter_eff)
    else:
        if not en.value(m.DHW):
            return m.E_loss_inv_PV[i] == (m.E_PV_grid[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i])+m.E_PV_batt_EV[i]*(1-m.Inverter_eff)/(m.Inverter_eff)
        else:
            return m.E_loss_inv_PV[i] == (m.E_PV_grid[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_PV_batt_EV[i])*(1-m.Inverter_eff)/(m.Inverter_eff)
...
def Inverter_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_loss_inv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
    else:
        if not en.value(m.DHW):
            return m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_inv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
        else:
            return m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_inv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
...
def Converter_rule(m,i):
    ...
    '''
    if not en.value(m.heating):
        return m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_loss_conv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
    else:
        if not en.value(m.DHW):
            return m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_conv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
        else:
            return m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_conv[i]+m.E_PV_batt_EV[i]<=m.Inverter_power*m.dt
...
def Obj_fcn(m):
    ...
    '''
    return sum((m.retail_price[i]*m.E_cons[i])+ (m.E_char_away[i]*m.public_charging_price)
    -(m.export_price[i]*(m.E_PV_grid[i]+m.E_batt_EV_grid[i])) for i in m.Time)*m.PVSC+(m.P_max_day*m.capacity_tariff)*m.DPS
