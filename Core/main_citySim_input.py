3# -*- coding: utf-8 -*-## @namespace main_beers
# Author
...
def load_data_from_city_sim():
     df = pd.read_csv('example_Jerome.csv')
     return df
...
######################################################
    ...
    if (conf!=0)&(conf!=1)&(conf!=4)&(conf!=5)&(conf!=8)&(conf!=9):#TS present
        ...
        if (combinations['house_type']=='SFH15')|(combinations['house_type']=='SFH45'):
            ...
            param['tank_sh']=pc.heat_storage_tank(mass=1500,surface=6)# For a 9600 liter tank with 3 m height and 2 m diameter 
            
        else:
            logger.debug('SHF 100')
            param['tank_sh']=pc.heat_storage_tank(mass=1500,surface=6) # For a 1500 liter tank with 1.426 m height and 1.25 diameter 
    ...
    if (conf==1)|(conf==3)|(conf==5)|(conf==7):#DHW present
        ...
        conf_aux[3]=True
        param['tank_dhw']=pc.heat_storage_tank(mass=200, t_max=60+273.15, t_min=40+273.15,surface=1.6564) # For a 200 liter tank with 0.95 m height and .555 diameter
    else:#No DHW
        logger.debug('No DHW')
        param['tank_dhw']=pc.heat_storage_tank(mass=0, t_max=0, t_min=0,specific_heat_dhw=0,U_value_dhw=0,surface_dhw=0)#null
