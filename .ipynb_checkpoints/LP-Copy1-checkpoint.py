...
def Concrete_model(Data):
    ...
    #Heating parameters
    m.HP_power=en.Param(initialize=Data['hp'].power_el)
    ...
    m.T_max=en.Param(initialize=Data['tank_sh'].t_max)# In K
    m.T_min=en.Param(initialize=Data['tank_sh'].t_min)# In K
    ...
    m.E_hp=en.Var(m.Time,bounds=(0,m.dt*m.HP_power),initialize=0)
    m.E_hp_cooling=en.Var(m.Time,bounds=(0,m.dt*m.HP_power),initialize=0)
    #m.COP=en.Var(m.Time,bounds=(0.01,None),initialize=1)
    #TS
    m.Q_ts_sh=en.Var(m.Time,bounds=(None,None),initialize=0)
    m.Q_hp_sh=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.T_ts=en.Var(m.tm,bounds=(m.T_min_cooling,m.T_max),initialize=m.T_init)#Temperature storage
    ...
    m.E_hpdhw=en.Var(m.Time,bounds=(0,m.dt*m.HP_power),initialize=0)
    ...
    m.T_dhwst=en.Var(m.tm,bounds=(m.T_min_dhw,m.T_max_dhw),initialize=m.T_min)#Temperature storage
    ...
    m.E_bu=en.Var(m.Time,bounds=(0,m.dt*m.HP_power),initialize=0)
    ...
    m.E_budhw=en.Var(m.Time,bounds=(0,m.dt*m.HP_power),initialize=0)
    #m.COP_SH=en.Var(m.Time,bounds=(0,None),initialize=7)
    #TS
    ...
    m.Balance_ts_r=en.Constraint(m.tm,rule=Balance_ts)#Verify if is necessary
    ...
    m.Balance_DHW_demand_r=en.Constraint(m.Time,
                                    rule=Balance_DHW_demand)
    ...
    #m.double_HP_r=en.Constraint(m.Time,rule=double_HP)
    ...
    #m.Q_dhwst_hd_r=en.Constraint(m.Time,rule=Q_dhwst_hd_rule)#Verify if is necessary
    ...
    if m.heating==False:
        ...
        if m.T_storage==False:
            ...
            if m.Cooling==False:
                if m.toy==1:
                    #return en.Constraint.Skip
                    return(sum(m.Q_ts_delta[i] for i in m.Time),(-20*m.m*m.c_p))
                elif m.toy==3:
                    return(sum(m.Q_ts_delta[i] for i in m.Time),(30*m.m*m.c_p))
                else:
                    return(m.Q_ts_delta[i],((m.T_ts[i]-m.T_ts[i-1])*m.m*m.c_p))
            else:
                return(m.Q_ts_delta[i],((m.T_ts[i]-m.T_ts[i-1])*m.m*m.c_p))

def Available_tank_thermal_energy_rule(m,t):
    '''
    Description
    -------
            ...
            if m.Cooling==False:
                if m.toy==1:
                    if t==-1:
                        ...
                        return (m.Q_ts[t],m.T_ts[t]-m.T_ts[t-1]-m.Q_ts_delta[t]/(m.m*m.c_p))
                elif m.toy==3:
                    if t==-1:
                        return (m.Q_ts[t],0)
                    else:
                        return (m.Q_ts[t],m.T_ts[t]-m.T_ts[t-1]-m.Q_ts_delta[t]/(m.m*m.c_p))
                        #return(m.Q_ts[t],((m.T_ts[t]-m.T_min)*m.m*m.c_p))#-20/96
                else:
                    return(m.Q_ts[t],((m.T_ts[t]-m.T_min)*m.m*m.c_p))
            else:
                return(m.Q_ts[t],((m.T_min-20-m.T_ts[t])*m.m*m.c_p))#return(m.Q_ts[i],((m.T_min-20-m.T_ts[i])m.m*m.c_p))
...
def Balance_ts(m,t):
    ...
    TODO
    -------
    En teoria los signos deberian ser iguales
    '''
    if m.heating==False:
        return en.Constraint.Skip
    else:
        if m.T_storage==False:
            ...
            if t==-1:
                ...
                if m.Cooling==False:
                    if m.toy==1:
                        #return en.Constraint.Skip
                        return (sum(m.E_hp_cooling[t]*(m.COP_SH[t]-1)for t in m.Time)
                        -sum(-m.Q_ts_delta[t]+m.Q_ts_sh[t]-m.Q_loss_ts[t] for t in m.Time)==False)#Balance (changed the sign)
                    else:
                        return (sum(m.E_hp[t]*m.COP_SH[t]+m.E_bu[t] for t in m.Time)
                    -sum(m.Q_ts_delta[t]+m.Q_ts_sh[t]+m.Q_loss_ts[t] for t in m.Time)==False)#Balance
                else:
                    return (sum(m.E_hp_cooling[t]*(m.COP_SH[t]-1)for t in m.Time)
                    +sum(m.Q_ts_delta[t]+m.Q_ts_sh[t]+m.Q_loss_ts[t] for t in m.Time)==False)#Balance (changed the sign)
...
#Not included
def Q_dhwst_hd_rule(m,i):
    '''
    Description
    -------
    Heat from DHWST <= Q_dis_DHWST
    TODO
    -------
    Verify if useful, probably not
    '''
    return(m.Q_dhwst_hd[i]<=(m.T_dhwst[i-1]-m.T_min_dhw)*m.c_p_dhw*m.m_dhw+m.E_hpdhw[i]*m.COP_DHW[i]+m.E_budhw[i])#Q_dis
...
def double_HP(m,i):
    ...
    Description
    -------
    If DHW two HPs are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used)
    TODO
    -------
    Verify if useful, probably not. Not used.
    '''
    if m.heating==False:
        return en.Constraint.Skip
    else:
        if m.DHW==False:
            ...
            if m.Cooling==False:
                return(m.E_hp[i]+m.E_hpdhw[i],max(m.E_hp[i],m.E_hpdhw[i]))
            else:
                return(m.E_hp_cooling[i]+m.E_hpdhw[i],max(m.E_hp_cooling[i],m.E_hpdhw[i]))
...
def Bool_hp_rule_1(m,i):
    ...
    if m.Cooling==False:
        if m.toy==1:
            return((m.E_hp_cooling[i])>=-bigM*(m.Bool_hp[i]))
        else:
...
def Bool_hp_rule_2(m,i):
    ...
    if m.Cooling==False:
        if m.toy==1:
            return((m.E_hp_cooling[i])<=bigM*(1-m.Bool_hpdhw[i]))
        else:
...
def double_bu(m,i):
    ...
    Description
    -------
    If DHW two HPs are used, thus, two backup heaters. One for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used)
    TODO
    -------
    Verify if useful, probably not. Not used.
    '''
    if m.heating==False:
        ...
        if m.DHW==False:
            ...
            if m.Cooling==False:
                return(m.E_bu[i]+m.E_budhw[i],max(m.E_bu[i],m.E_budhw[i]))
            else:
                return en.Constraint.Skip

def Bool_bu_rule_1(m,i):
    '''
    Description
    -------
    If DHW two backup heaters are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used). BigM method to solve linear programming models using simplex algorithm.1/4
    '''
    bigM=500000
    if m.Cooling==False:
        if m.toy==1:
            ...
            return((m.E_bu[i])>=-bigM*(m.Bool_bu[i]))
    else:
        return en.Constraint.Skip

def Bool_bu_rule_2(m,i):
    '''
    Description
    -------
    If DHW two backup heaters are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used). BigM method to solve linear programming models using simplex algorithm.2/4
    '''
    bigM=500000
    if m.Cooling==False:
        if m.toy==1:
            ...
            return((m.E_bu[i])<=bigM*(1-m.Bool_budhw[i]))
    else:
        return en.Constraint.Skip

def Bool_bu_rule_3(m,i):
    '''
    Description
    -------
    If DHW two backup heaters are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used). BigM method to solve linear programming models using simplex algorithm.3/4
    '''
    bigM=500000
    return((m.E_budhw[i])>=-bigM*(m.Bool_budhw[i]))

def Bool_bu_rule_4(m,i):
    '''
    Description
    -------
    If DHW two backup heaters are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used). BigM method to solve linear programming models using simplex algorithm.4/4
    '''
    bigM=500000
    return((m.E_budhw[i])<=bigM*(1-m.Bool_bu[i]))

def Bu_1_2(m,i):
    '''
    Description
    -------
    If DHW two backup heaters are used, one for heating/cooling and one for DHW. However, both cannot operate at the same time (in real world only one HP is used). BigM method to solve linear programming models using simplex algorithm. Restriction to operate only one at time.
    '''
    return (m.Bool_bu[i]+m.Bool_budhw[i],1)
###########################################################################
