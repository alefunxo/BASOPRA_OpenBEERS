...
def Concrete_model(Data):
    ...
    m.ht=en.Param(initialize=Data['aux_ht'])
    ...
    m.E_bu=en.Var(m.Time,bounds=(0,Data['Backup_heater']),initialize=0)
    ...
    m.E_budhw=en.Var(m.Time,bounds=(0,Data['Backup_heater']),initialize=0)
...
def Balance_hp_supply_rule(m,i):
    ...
    if m.heating==False:
        ...
        if m.T_storage==False:
            ...
            return((m.E_hp[i]+m.E_bu[i]/m.COP_SH[i]),(m.Q_hp_sh[i])/m.COP_SH[i])

        else:
            return ((m.E_hp[i]+m.E_bu[i]/m.COP_tank[i]),m.Q_hp_ts[i]/m.COP_tank[i])#+m.Q_hp_sh[i]/m.COP_SH[i])
...
def Balance_hp_power(m,i):
    ...
    if m.heating==False:
        ...
        if m.T_storage==False:
            return((m.E_hp[i])<=m.HP_power_SH[i])
        else:
            return((m.E_hp[i])<=m.HP_power_tank[i])
